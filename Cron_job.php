...
class Cron_job extends Home
{
  ...
  /*  public function text_image_link_video_post($api_key="") //publish_post
    ...
    public function text_image_link_video_post($api_key="") //publish_post
    {
        ...
        $post_info= $this->basic->get_data("facebook_rx_auto_post",$where,$select='',$join='',$limit=2, $start=0, $order_by='schedule_time ASC');
        ...
        foreach($post_info as $info)
        {
            ...
            else if($post_type=="image_submit")
            {
             ...
             else if(isset($reels_post) && $reels_post == 'yes') 
                     {
                            $response = $this->fb_rx_login->uploadReel($message, $video_url, $use_access_token, $use_fb_id);
                             sleep(rand ( 10 , 20 ));
                        } 
            else
            {
            ...
            if($post_type=="image_submit")
            {
             ...
             $object_id = isset($response["id"]) ? $response["id"] : (isset($response["post_id"]) ? $response["post_id"] : "");
            ...
            try
            {
                ...
                {
                    $temp_data = "https://www.facebook.com/$object_id";
                }
                else 
                {
	               $temp_data=$this->fb_rx_login->get_post_permalink($object_id,$use_access_token);
            ...
            {
                $post_url= "https://www.facebook.com/$object_id";
            }
            else 
            {
	        $post_url= isset($temp_data["permalink_url"]) ? $temp_data["permalink_url"] : "";
...
$log_file = '/home/ashmawytech/public_html/response_log.txt';

// التحقق من الـ post_id و الرسائل
$post_id1 = $object_id;
$post_id = $post_id1;
$messages = [
    $info['comment1'],
...
// التحقق من أن الـ page_access_token و messagess غير فارغين
if (!empty($use_access_token) && !empty($messages)) {
    try {
        // تنفيذ التعليق التلقائي 5 مرات برسائل مختلفة من POST
        foreach ($messages as $index => $messageq) {
            

            if (!empty($messageq)) {  // التأكد من أن الرسالة غير فارغة
                    $image_url = !empty($image_urls[$index]) ? $image_urls[$index] : null;
                $response = $this->fb_rx_login->auto_comment($messageq, $object_id, $use_access_token, $image_url);
